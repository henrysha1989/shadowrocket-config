...
* **文件**：两份，规则逻辑同源 ——
  * [`shadowrocket-白名单.conf`](./shadowrocket-白名单.conf)：**作者自用版（默认）**。60 条规则、23 条远端规则集，含自建 DoH 与自建加速站。
  * [`shadowrocket-白名单.通用版.conf`](./shadowrocket-白名单.通用版.conf)：**通用版，任何人可直接用**。56 条规则、20 条远端规则集，换公共 DoH + 公共 CDN jsDelivr。
